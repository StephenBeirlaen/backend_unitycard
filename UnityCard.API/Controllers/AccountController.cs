...
namespace UnityCard.API.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        private IExternalAuthRepository repoExternalAuth;

        public AccountController()
        {
        }

        public AccountController(ILoyaltyCardRepository repoLoyaltyCards, IExternalAuthRepository repoExternalAuth)
        {
            this.repoLoyaltyCards = repoLoyaltyCards;
            this.repoExternalAuth = repoExternalAuth;
        }
        ...
        // GET api/Account/ExternalLoginSuccess
        [HttpGet]
        [OverrideAuthentication]
        [HostAuthentication(DefaultAuthenticationTypes.ExternalCookie)]
        [AllowAnonymous]
        [Route("ExternalLoginSuccess", Name = "ExternalLoginSuccess")]
        public IHttpActionResult ExternalLoginSuccess()
        {
            return Ok();
        }
        ...
        private async Task<ParsedExternalAccessToken> VerifyExternalAccessToken(string provider, string accessToken)
        {
            ...
            if (provider == "Facebook")
            {
                ...
                var appToken = "339748679723003|tjMdagAZMsxdfj7BNNIjY8OWPaM";
        ...
        public async Task<IHttpActionResult> RegisterExternal(RegisterExternalBindingModel model)
        {
            ...
            result = await repoExternalAuth.AddLoginAsync(user.Id, info.Login);
            if (!result.Succeeded)
            {
                return GetErrorResult(result);
            }
        ...
        public async Task<IHttpActionResult> ObtainLocalAccessToken(string provider, string externalAccessToken)
        {
            ...
            IdentityUser user = await repoExternalAuth.FindAsync(new UserLoginInfo(provider, verifiedAccessToken.user_id));
        ...
        public async Task<IHttpActionResult> ChangeFcmToken(ChangeFcmTokenBindingModel model)
        {
            ...
            UserStore<ApplicationUser> userStore = new UserStore<ApplicationUser>(new UnityCardDbContext());
